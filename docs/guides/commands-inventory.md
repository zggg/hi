...
> 状态：**草案** — 供审核是否收敛、合并、删除。  
> 最后更新：2026-06-23（M7 关闭；memory_write / memory_search 已落地）
...
| `/verbose` | ✗ | ✓ | 切换详细模式：think 与工具 output 全文流式展开（再次输入关闭） |
...
**当前数量**：共享 5 个字符串（3 个有效 + 2 别名）；TUI 额外 `/model` `/verbose`。

> 详细模式也可由 CLI 启动开关：`hi -v` / `hi --verbose` / `hi tui -v`（全局参数，等价于进入后立即 `/verbose`）。
...
| 类别 | 条目数 | 日常对话典型用量 |
|------|--------|------------------|
| 对话斜杠 | 7（共享 3 有效 + 2 别名 + TUI `/model` `/verbose`） | 0～1 |
| TUI 快捷键 | ~12 | 2～3 |
